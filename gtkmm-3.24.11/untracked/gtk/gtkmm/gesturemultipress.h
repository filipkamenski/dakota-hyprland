 ...
 * its own.
 *
 * %GestureMultiPress is a subclass of GestureSingle. It can handle only one pressed
 * button at a time. If another button is pressed while one button is kept
 * pressed, the event sequence may be cancelled, and no signal_released()
 * may be emitted. To detect such a situation, connect to Gesture::signal_cancel().
 *
 * @newin{3,14}
 *
 * @ingroup Gestures
